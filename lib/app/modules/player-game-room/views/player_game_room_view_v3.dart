...
import '../controllers/player_game_room_controller_v3.dart';

/// PlayerGameRoomView V3 - Fully synchronized with Web app EventPlayerPage
/// 
...
class PlayerGameRoomViewV3 extends StatelessWidget {
  const PlayerGameRoomViewV3({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(PlayerGameRoomControllerV3());
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        leading: IconButton(
          onPressed: () => _showLeaveGameConfirm(context, controller),
          icon: Icon(Icons.arrow_back_ios, color: ColorsManager.primary),
        ),
        actions: [
          // Nút Rời game ở header
          Obx(() {
            // Chỉ hiển thị nút "Rời game" khi đang trong game (không phải finalResult)
            if (controller.gamePhase.value == 'finalResult') {
  ...
  Widget _buildCurrentPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
  ...
  void _showLeaveGameConfirm(BuildContext context, PlayerGameRoomControllerV3 controller) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          title: TextConstant.titleH2(
            context,
            text: "Xác nhận rời game",
            color: Colors.black,
            fontWeight: FontWeight.bold,
          ),
          content: TextConstant.subTile1(
            context,
            text: "Bạn có chắc chắn muốn rời khỏi game? Tiến trình của bạn sẽ không được lưu.",
  ...
  Widget _buildEnterPlayerName(BuildContext context, PlayerGameRoomControllerV3 controller) {
  ...
  Widget _buildConnectingPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
    return SingleChildScrollView(
      padding: EdgeInsets.all(UtilsReponsive.width(20, context)),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
  ...
  // ==================== CONNECTING PHASE ====================
  // (Already defined above - this duplicate has been removed)

  // ==================== LOBBY PHASE ====================

  Widget _buildLobbyPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
    return SingleChildScrollView(
      padding: EdgeInsets.all(UtilsReponsive.width(16, context)),
      child: Column(
  ...
  Widget _buildCountdownPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
    return Container(
      width: double.infinity,
  ...
  Widget _buildPlayingPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
    ...
    return Column(
      children: [
        // Nút Rời game đã được di chuyển lên header
  ...
  Widget _buildBossHPBar(BuildContext context, PlayerGameRoomControllerV3 controller) {
  ...
  Widget _buildGroupItem(BuildContext context, PlayerGameRoomControllerV3 controller) {
  ...
  Widget _buildAnswerOptions(
    BuildContext context,
    PlayerGameRoomControllerV3 controller,
  ...
  Widget _buildAnsweredPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
  ...
  Widget _buildFinalResultPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
  ...
  Widget _buildErrorPhase(BuildContext context, PlayerGameRoomControllerV3 controller) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
